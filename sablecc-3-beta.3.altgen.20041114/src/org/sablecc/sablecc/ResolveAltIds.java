...
public class ResolveAltIds extends DepthFirstAdapter
{
  ...
  public void caseAProd(AProd node)
  {
    ...
    {
      if(temp[i].getUnOp() != null && temp[i].getUnOp() instanceof AQMarkUnOp)
      {
        error(((AQMarkUnOp)temp[i].getUnOp()).getQMark());
      }

      if(temp[i].getUnOp() != null && temp[i].getUnOp() instanceof APlusUnOp)
      {
        error(((APlusUnOp)temp[i].getUnOp()).getPlus());
      }
    }
    */
   ...
   * is created.
   */
  public void caseAParsedAlt(AParsedAlt alt)
  {
  ...
  private static void error(Token token)
  {
    throw new RuntimeException(
      ...
      "Question mark and Plus operators are not permit for an element of production transformation");
  }
  */
