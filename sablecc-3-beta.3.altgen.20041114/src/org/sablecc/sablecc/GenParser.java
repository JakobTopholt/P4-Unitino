...
public class GenParser extends DepthFirstAdapter
{
  ...
  public void caseStart(Start tree)
  {
    tree.getPGrammar().apply(new DepthFirstAdapter()
                             {
                               public void caseAProd(AProd node)
                               {
                                 ...
                                 {
                                   List ptc = new java.util.LinkedList(node.getProdTransform());
	...
	//Here, we are trying to inline the grammar with production imply in the conflict.
        if(processInlining)
        {
          ComputeInlining compute = new ComputeInlining(ce.getConflictualProductions(),
                                    mapOfProds.productionsMap,
                                    tree);

          if(!compute.computeInlining())
          {
            tree.apply(new PrettyPrinter());
    ...
    tree.apply(new DepthFirstAdapter()
               {
                 ...
                 public void caseAParsedAlt(final AParsedAlt alt)
                 {
                 ...
                 public void caseAElem(final AElem elem)
                 {
                   if(elem.getElemName() != null)
                   {
                     String name = currentAlt + "." +
                                   ids.name(elem.getElemName().getText());
  ...
  private void createParser()
  {
	...
	macros.apply(file, "ParserCommon", new String[] {"", ""}); 
      ...
      for(int i = 0; i < (productions.length - 1); i++)
      {
        final ParserData.Rule rule = new ParserData.Rule (productions[i].name,
          productions[i].index, productions[i].leftside);
        ids.data.rules_list.add(rule);
        ...
        try
        {
          for(Iterator it = stack.iterator(); it.hasNext();)
          {
            ...
            rule.actions.add (new ParserData.Action (ParserData.Action.POP,
                                                     "nodeArrayList" + e.arguments[0],
                                                     -1, null, null));
          }
        }
        catch(IOException e)
      ...
      ids.data.parser_action_table = outerArray;
      ...
      ids.data.parser_goto_table = outerArray;
      ...
      ids.data.parser_errors = indexArray;
      ids.data.parser_error_messages = outerArray;
