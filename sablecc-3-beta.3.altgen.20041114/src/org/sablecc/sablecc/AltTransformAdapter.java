...
public class AltTransformAdapter extends DepthFirstAdapter
{
  ...
  public void inASimpleTerm(ASimpleTerm node)
  {
    ...
    if( !((List)altIds.alts_elems.get(currentNewAltName)).contains(name) &&
        !((List)altIds.alts_elems.get(currentNewAltName)).contains("$"+name) )
    {
  ...
  public void inASimpleListTerm(ASimpleListTerm node)
  {
    ...
    if( isElementIsAlist.get(currentNewAltName+name) != null)
    {
      ...
      TId tid;
      tid = ( ((List)altIds.alts_elems_list_elemName.get(currentNewAltName)).contains(name) ?
