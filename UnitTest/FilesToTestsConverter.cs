...
public static class FilesToTestsConverter
{
    ...
    private static List<Data> FileToTestCases()
    {
        ...
        lock (ThreadLock)
        {
            if (TestData.Count > 0)
                return TestData;
            IEnumerable<Task> tasks = Directory.GetFiles(Directory.GetCurrentDirectory() + "/../../../Testcases")
                .Select(file => new Task(() =>
                {
                    ...
                    foreach (string oneTestCaseString in testCasesString.Split("//////"))
                    {
                        ...
                        TestData.Add(new Data(ast, prettyPrint, codeGen, name, error, typedCorrect));
                    }
                }));
